...
export async function  DELETE (request) {
  const client = await connect
  const body = await request.json()
  await client.db("ccchs-cubs").collection("attendance").deleteOne(body.rosterId)
  return Response.json({message: "successfully updated the document"})
}
...
export async function PUT(request) {
  ...
  try {
    ...
    const document = await client.db("ccchs-cubs")
    ...
    const index = document.attendanceRecord.findIndex(record =>
    ...
    if (index === -1) {
      return new Response(JSON.stringify({ message: "Record not found in attendanceRecord" }), { status: 404 });
    }
    ...
    const result = await client.db("ccchs-cubs")
      .collection("attendance")
      .updateOne(
        { "attendanceRecord.rosterId": body.rosterId, [`attendanceRecord.${index}.day`]: body.day, [`attendanceRecord.${index}.month`]: body.month },
        { $set: { [`attendanceRecord.${index}.present`]: body.present } }
      );

    if (result.modifiedCount > 0) {
      return new Response(JSON.stringify({ message: "Successfully updated the document" }), { status: 200 });
    } else {
